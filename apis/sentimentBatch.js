...
let sentimentBatch = function(text_list,lang_code,API_KEY){
	return new Promise((resolve,reject) => {
		...
		request.post({url:'http://apis.paralleldots.com/v3/sentiment_batch', form: {text:JSON.parse(text),lang_code:lang_code,api_key:API_KEY}}, function(err,httpResponse,body){ 
			if(err){
				reject({"Error":err});
			}
			resolve(body);
